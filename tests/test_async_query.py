...
class TestAsyncQuery(tb.AsyncQueryTestCase):
    ...
    async def test_async_parse_error_recover_01(self):
        for _ in range(2):
            ...
            with self.assertRaisesRegex(
                edgedb.EdgeQLSyntaxError, "Unexpected end of line"
            ):
            ...
            for _ in range(10):
                self.assertEqual(
                    await self.client.query("select 1;"), edgedb.Set((1,))
                )
    ...
    async def test_async_exec_error_recover_03(self):
        ...
        for i in [1, 2, 0, 3, 1, 0, 1]:
            if i:
                self.assertEqual(
                    await self.client.query(query, i), edgedb.Set([10 // i])
                )
            else:
                with self.assertRaises(edgedb.DivisionByZeroError):
    ...
    async def test_async_exec_error_recover_05(self):
        with self.assertRaises(edgedb.DivisionByZeroError):
            await self.client.execute("select 1 / 0")
        self.assertEqual(await self.client.query('SELECT "HELLO"'), ["HELLO"])
    ...
    async def test_async_basic_datatypes_01(self):
        for _ in range(10):
            ...
            self.assertEqual(
                await self.client.query('select ["a", "b"]'),
                edgedb.Set([["a", "b"]]),
            )
    ...
    async def test_async_basic_datatypes_02(self):
        ...
        self.assertEqual(
            await self.client.query(r"select <bytes>$0", b"he\x00llo"),
            edgedb.Set([b"he\x00llo"]),
        )

    async def test_async_basic_datatypes_03(self):
        for _ in range(10):  # test opportunistic execute
            self.assertEqual(await self.client.query_json("select ()"), "[[]]")

            self.assertEqual(
                await self.client.query_json("select (1,)"), "[[1]]"
            )

            self.assertEqual(
                await self.client.query_json("select <array<int64>>[]"), "[[]]"
            )

            self.assertEqual(
                json.loads(await self.client.query_json('select ["a", "b"]')),
                [["a", "b"]],
            )

            self.assertEqual(
                json.loads(
                    await self.client.query_single_json('select ["a", "b"]')
                ),
                ["a", "b"],
            )
            ...
            self.assertEqual(
                json.loads(await self.client.query_json("SELECT {1, 2}")),
                [1, 2],
            )
    ...
    async def test_async_args_03(self):
        ...
        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "None is not allowed"
        ):
            await self.client.query("select <array<int64>>$0", [1, None, 3])
    ...
    async def test_async_args_04(self):
        ...
        self.assertEqual(
            await self.client.query_single(
                "select <datetime>$0;", aware_datetime
            ),
            aware_datetime,
        )
        ...
        self.assertEqual(
            await self.client.query_single(
                "select <cal::local_datetime>$0;", naive_datetime
            ),
            naive_datetime,
        )
        ...
        self.assertEqual(
            await self.client.query_single(
                "select <cal::local_time>$0;", naive_time
            ),
            naive_time,
        )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single(
                "select <datetime>$0;", naive_datetime
            )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single(
                "select <cal::local_time>$0;", aware_time
            )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single(
                "select <cal::local_datetime>$0;", aware_datetime
            )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single(
                "select <cal::local_datetime>$0;", date
            )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single("select <datetime>$0;", date)
    ...
    async def _test_async_args_05(self):  # XXX move to edgedb/edgedb
        ...
        self.assertEqual(await self.client.query("select <int32>$a", a=1), [1])
        self.assertEqual(
    ...
    async def _test_async_args_06(self):  # XXX move to edgedb/edgedb
        ...
        self.assertEqual(
            await self.client.query("select <optional int32>$a", a=1), [1]
        )
    ...
    async def test_async_args_bigint_basic(self):
        ...
        val = await self.client.query_single(
            "select <array<bigint>>$arg", arg=testar
        )

        self.assertEqual(testar, val)

    async def test_async_args_bigint_pack(self):
        val = await self.client.query_single("select <bigint>$arg", arg=10)
        self.assertEqual(val, 10)

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query("select <bigint>$arg", arg="bad int")

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query("select <bigint>$arg", arg=10.11)

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query(
                "select <bigint>$arg", arg=decimal.Decimal("10.0")
            )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query(
                "select <bigint>$arg", arg=decimal.Decimal("10.11")
            )

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query("select <bigint>$arg", arg="10")

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query_single(
                "select <bigint>$arg", arg=decimal.Decimal("10")
            )
    ...
    async def test_async_args_intlike(self):
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single("select <int16>$arg", arg=IntLike())

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query_single("select <int32>$arg", arg=IntLike())

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected an int"
        ):
            await self.client.query_single("select <int64>$arg", arg=IntLike())
    ...
    async def test_async_args_decimal(self):
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single(
                "select <decimal>$arg", arg=IntLike()
            )

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, "expected a Decimal or an int"
        ):
            await self.client.query_single("select <decimal>$arg", arg="10.2")
        ...
        samples = [
            (
                ...
                [
                    ...
                    dict(
                        input=edgedb.Range(
                            1, 2, inc_lower=True, inc_upper=True
                        ),
                        output=edgedb.Range(
                            1, 3, inc_lower=True, inc_upper=False
                        ),
                    ),
                    edgedb.Range(empty=True),
                    dict(
                        input=edgedb.Range(
                            1, 1, inc_lower=True, inc_upper=False
                        ),
    ...
    async def test_async_range_02(self):
        ...
        result = await self.client.query_single(
            "SELECT <array<range<int32>>>$0", [edgedb.Range(1, 2)]
        )
    ...
    async def test_async_wait_cancel_01(self):
        ...
        ).with_retry_options(RetryOptions(attempts=1))
    ...
    async def test_enum_argument_01(self):
        ...
        self.assertEqual(
            await self.client.query_single("SELECT <MyEnum>$0", "A"), A
        )
    ...
    async def test_async_cancel_01(self):
        ...
        try:
            ...
            with self.assertRaises(asyncio.TimeoutError):
                await asyncio.wait_for(
                    client.query_single("SELECT sys::_sleep(10)"), timeout=0.1
                )
