...
class TestSyncQuery(tb.SyncQueryTestCase):
    ...
    def test_sync_parse_error_recover_01(self):
        for _ in range(2):
            ...
            for _ in range(10):
                self.assertEqual(
                    self.client.query("select 1;"), edgedb.Set((1,))
                )
    ...
    def test_sync_exec_error_recover_03(self):
        ...
        for i in [1, 2, 0, 3, 1, 0, 1]:
            if i:
                self.assertEqual(
                    self.client.query(query, i), edgedb.Set([10 // i])
                )
            else:
                with self.assertRaises(edgedb.DivisionByZeroError):
    ...
    def test_sync_basic_datatypes_01(self):
        for _ in range(10):
            ...
            self.assertEqual(
                self.client.query_single("select <array<int64>>[]"), []
            )

            self.assertEqual(
                self.client.query('select ["a", "b"]'),
                edgedb.Set([["a", "b"]]),
            )
    ...
    def test_sync_basic_datatypes_03(self):
        for _ in range(10):
            ...
            self.assertEqual(
                self.client.query_json("select <array<int64>>[]"), "[[]]"
            )

            self.assertEqual(
                json.loads(self.client.query_json('select ["a", "b"]')),
                [["a", "b"]],
            )
