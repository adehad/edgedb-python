...
test_uuids = tuple(
    special_uuids | frozenset({uuid.uuid4() for _ in range(1000)})
)
...
class TestUuid(unittest.TestCase):
    ...
    def test_uuid_ctr_01(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"invalid UUID.*decodes to less"
        ):
        ...
        for v in {
            ...
            with self.assertRaisesRegex(
                ValueError, r"invalid UUID.*decodes to more"
            ):
                print(c_UUID(v))

        with self.assertRaisesRegex(
            ValueError, r"invalid UUID.*unexpected.*'x'"
        ):
